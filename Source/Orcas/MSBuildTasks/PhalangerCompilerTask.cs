...
namespace PHP.VisualStudio.PhalangerTasks
{
	public class PhalangerCompilerTask : Task
	{
		...
		public override bool Execute()
		{
			...
			if (references != null)
			{
                foreach (ITaskItem assemblyReference in references)
                {
		...
		{
            // create new AppDomain every time, referenced assemblies could be changed
            // the AppDomain has ShadowCopyFiles enabled to not lock loaded assemblies
            ApplicationCompiler remoteCompiler = ApplicationCompiler.CreateRemoteCompiler();

            // perform the compilation process in the separated AppDomain
