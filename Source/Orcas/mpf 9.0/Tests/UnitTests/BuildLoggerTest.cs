...
namespace Microsoft.VisualStudio.Project.UnitTests
{
	internal class IDELoggerProxy
	{
		...
		private static MethodInfo setIsLogTaskDoneInfo;
		public bool IsLogTaskDone
		{
			get { return false; }
			set
			{
				if(null == setIsLogTaskDoneInfo)
				{
					setIsLogTaskDoneInfo = IDEBuildLoggerType.GetMethod("set_IsLogTaskDone", BindingFlags.Public | BindingFlags.Instance);
				}
				setIsLogTaskDoneInfo.Invoke(loggerInstance, new object[] { value });
			}
		}
		...
		public void MessageFormattingTest()
		{
			...
			using(OleServiceProvider provider = new OleServiceProvider())
			{
				using(TaskProvider task = new TaskProvider(new ServiceProvider(provider)))
				{
					...
					// As before, but now verify that if the IsLogTaskDone is not set, then no end task message is written
					logger.IsLogTaskDone = false;
					output.Clear();
					startTaskArgs = new TaskStartedEventArgs(startText, null, null, null, null);
					taskStartedHandler.Invoke(mockSource, startTaskArgs);
					message = new BuildMessageEventArgs(messageText, "help", "sender", MessageImportance.Normal);
					messageHandler.Invoke(mockSource, message);
					endTaskArgs = new TaskFinishedEventArgs(endText, null, null, null, null, true);
					taskFinishedHandler.Invoke(mockSource, endTaskArgs);

					// Verify the text in the output pane.
					expected = string.Format("{0}{1}\t{2}{1}", startText, Environment.NewLine, messageText, endText);
					builder = (System.Text.StringBuilder)mockOutput["StringBuilder"];
					Assert.AreEqual(expected, builder.ToString(), false);
				}
			}
