	...
	public class TestProject
	{
		...
		public void TestUnloadReloadOfProject()
		{
			UIThreadInvoker.Invoke((ThreadInvoker)delegate()
			{
				...
				Assert.IsTrue(ourHierarchy is IProjectEventsListener, "Our hierachy has not been relaoded successfully");
		...
		private void VerifyNestedProjectRename(ProjectNode project, NestedProjectNode nestedProject, IVsHierarchy nestedHierarchy, string testName)
		{
			...
			foreach(MSBuild.BuildItem item in buildProject.EvaluatedItems)
			{
				if(String.Compare(item.Name, ProjectFileConstants.SubProject, StringComparison.OrdinalIgnoreCase) == 0)
				{
					string name = Path.GetFileNameWithoutExtension(item.Include);
