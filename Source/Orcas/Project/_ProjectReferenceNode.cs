...
namespace PHP.VisualStudio.PhalangerProject
{
	...
	/// </summary>
	public class PhalangerProjectReferenceNode : ProjectReferenceNode
	{
		public PhalangerProjectReferenceNode(ProjectNode root, ProjectElement element)
			: base(root, element)
		{
		}

		public PhalangerProjectReferenceNode(ProjectNode project, string referencedProjectName, string projectPath, string projectReference)
		...
		private bool HasFormItems()
		{
			...
			this.ProjectMgr.FindNodesOfType<PhalangerFileNode>(pythonFileNodes);
			foreach (PhalangerFileNode node in pythonFileNodes)
			{
		...
		private void ShowProjectReferenceErrorMessage()
		{
			...
			{
				string message = "ProjectReferenceError"; // SR.GetString(SR.ProjectReferenceError, CultureInfo.CurrentUICulture);
		...
		private void ShowProjectReferenceErrorMessage2()
		{
			if (!Utilities.IsInAutomationFunction(this.ProjectMgr.Site))
			{
				string message = "ProjectReferenceError2"; //SR.GetString(SR.ProjectReferenceError2, CultureInfo.CurrentUICulture);
				string title = string.Empty;
				OLEMSGICON icon = OLEMSGICON.OLEMSGICON_CRITICAL;
				OLEMSGBUTTON buttons = OLEMSGBUTTON.OLEMSGBUTTON_OK;
				OLEMSGDEFBUTTON defaultButton = OLEMSGDEFBUTTON.OLEMSGDEFBUTTON_FIRST;
