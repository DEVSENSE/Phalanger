...
namespace PHP.VisualStudio.PhalangerLanguageService {
    ...
    /// </summary>
    [Guid(PhalangerConstants.libraryManagerServiceGuidString)]
    public interface IPhalangerLibraryManager {
    ...
    /// hierarchy.
    /// </summary>
    [Guid(PhalangerConstants.libraryManagerGuidString)]
    internal class PhalangerLibraryManager : IPhalangerLibraryManager, IVsRunningDocTableEvents, IDisposable 
		{
			public PhalangerLibraryManager(IServiceProvider provider)
			{
			}
        ...
        private Dictionary<uint, _TextLineEventListener> documents;
        private Dictionary<IVsHierarchy, _HierarchyListener> hierarchies;
        private Dictionary<ModuleId, _LibraryNode> files;
        ...
        public PhalangerLibraryManager(IServiceProvider provider) {
            documents = new Dictionary<uint, _TextLineEventListener>();
            hierarchies = new Dictionary<IVsHierarchy, _HierarchyListener>();
            library = new _Library(new Guid("0925166e-a743-49e2-9224-bbe206545104"));
            library.LibraryCapabilities = (_LIB_FLAGS2)_LIB_FLAGS.LF_PROJECT;
            files = new Dictionary<ModuleId, _LibraryNode>();
        ...
        public void Dispose() {
            ...
            foreach (_HierarchyListener listener in hierarchies.Values) {
                listener.Dispose();
            }
            hierarchies.Clear();

            foreach(_TextLineEventListener textListener in documents.Values) {
        ...
        #region IPhalangerLibraryManager
        public void RegisterHierarchy(IVsHierarchy hierarchy) {
            ...
            _HierarchyListener listener = new _HierarchyListener(hierarchy);
        ...
        public void UnregisterHierarchy(IVsHierarchy hierarchy) {
            ...
            _HierarchyListener listener = hierarchies[hierarchy];
            if (null != listener) {
                listener.Dispose();
            }
            ...
            foreach (uint id in docKeys) {
                _TextLineEventListener docListener = documents[id];
        ...
        private void ParseThread() {
            ...
            while (true) {
                ...
                _LibraryNode module = new _LibraryNode(
                        System.IO.Path.GetFileName(task.FileName),
                        _LibraryNode.LibraryNodeType.PhysicalContainer);
                CreateModuleTree(module, module, scope, "", task.ModuleID);
                if (null != task.ModuleID) {
                    _LibraryNode previousItem = null;
                    lock (files) {
        ...
        private void CreateModuleTree(_LibraryNode root, _LibraryNode current, ScopeNode scope, string namePrefix, ModuleId moduleId)
        {
            ...
            foreach (ScopeNode subItem in scope.NestedScopes) {
                ...
                // current node.
                if ((newNode.NodeType & _LibraryNode.LibraryNodeType.Members) != _LibraryNode.LibraryNodeType.None) {
                    current.AddNode(newNode);
                } else if ((newNode.NodeType & _LibraryNode.LibraryNodeType.Classes) != _LibraryNode.LibraryNodeType.None) {
            ...
            if ((grfAttribs & (uint)(__VSRDTATTRIB.RDTA_MkDocument)) == (uint)__VSRDTATTRIB.RDTA_MkDocument) {
                ...
                if (rdt != null) {
                    ...
                    try {
                        ...
                        _TextLineEventListener listner;
        ...
        public int OnBeforeDocumentWindowShow(uint docCookie, int fFirstShow, IVsWindowFrame pFrame)
        {
            ...
            if (null != rdt) {
                ...
                try {
                    ...
                    if (0 != string.Compare(extension, PhalangerConstants.phpFileExtension, StringComparison.OrdinalIgnoreCase)) {
                        return VSConstants.S_OK;
                    }
                    ...
                    // Create the listener.
                    _TextLineEventListener listener = new _TextLineEventListener(buffer, documentMoniker, docId);
        ...
        public int OnBeforeLastDocumentUnlock(uint docCookie, uint dwRDTLockType, uint dwReadLocksRemaining, uint dwEditLocksRemaining)
        {
            ...
            _TextLineEventListener listener;
        ...
        public void OnIdle() {
            foreach(_TextLineEventListener listener in documents.Values) {
                listener.OnIdle();
            }
        }
			 */

			#region Phalanger - NOT IMPLEMENTED
			...
			public void RegisterLineChangeHandler(uint document, TextLineChangeEvent lineChanged, Action<IVsTextLines> onIdle)
			{
			}
