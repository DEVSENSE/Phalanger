...
namespace PHP.VisualStudio.PhalangerLanguageService {
    ...
    /// </summary>
    internal class _LibraryNode : IVsSimpleObjectList2, IVsNavInfoNode {
        ...
        private List<_LibraryNode> children;
        ...
        private Dictionary<LibraryNodeType, _LibraryNode> filteredView;

        public _LibraryNode(string name)
            : this(name, LibraryNodeType.None, LibraryNodeCapabilities.None, null)
        { }
        public _LibraryNode(string name, LibraryNodeType type)
            : this(name, type, LibraryNodeCapabilities.None, null)
        { }
        public _LibraryNode(string name, LibraryNodeType type, LibraryNodeCapabilities capabilities, CommandID contextMenuID) {
            this.capabilities = capabilities;
            this.contextMenuID = contextMenuID;
            this.name = name;
            this.tooltip = name;
            this.type = type;
            children = new List<_LibraryNode>();
            clipboardFormats = new List<VSOBJCLIPFORMAT>();
            filteredView = new Dictionary<LibraryNodeType, _LibraryNode>();
        }
        public _LibraryNode(_LibraryNode node) {
            ...
            this.type = node.type;
            this.children = new List<_LibraryNode>();
            foreach (_LibraryNode child in node.children) {
                children.Add(child);
            }
            ...
            this.filteredView = new Dictionary<LibraryNodeType, _LibraryNode>();
        ...
        internal void AddNode(_LibraryNode node) {
            lock (children) {
                children.Add(node);
            }
            updateCount += 1;
        }
        internal void RemoveNode(_LibraryNode node) {
            lock (children) {
                ...
                case LIB_CATEGORY.LC_LISTTYPE:
                    {
                        ...
                        foreach (_LibraryNode node in children) {
                            subTypes |= node.type;
                        }
        ...
        protected virtual _LibraryNode Clone() {
            return new _LibraryNode(this);
        }
