...
namespace PHP.Core.Compiler.AST
{
    partial class NodeCompilers
    {
        ...
        abstract class ConstantUseCompiler : ExpressionCompiler<ConstantUse>
        {
            protected DConstant constant;

            internal abstract void ResolveName(ConstantUse/*!*/node, Analyzer/*!*/ analyzer);

            /// <summary>
            /// Determines behavior on assignment.
            /// </summary>
            /// <returns>Always <B>false</B>, since constants contain immutable objects only.</returns>
            public override bool IsDeeplyCopied(ConstantUse node, CopyReason reason, int nestingLevel)
            {
                return false;
            }

            public override Evaluation Analyze(ConstantUse node, Analyzer analyzer, ExInfoFromParent info)
            {
        ...
        sealed class GlobalConstUseCompiler : ConstantUseCompiler
        {
            public override Evaluation EvaluatePriorAnalysis(ConstantUse node, SourceUnit sourceUnit)
            {
                var/*!*/constuse = (GlobalConstUse)node;

                constant = sourceUnit.TryResolveGlobalConstantGlobally(constuse.Name);
                return (constant != null && constant.HasValue) ? new Evaluation(node, constant.Value) : new Evaluation(node);
            }

            internal override void ResolveName(ConstantUse/*!*/node, Analyzer/*!*/ analyzer)
            {
                if (constant == null)
                    constant = analyzer.ResolveGlobalConstantName(((GlobalConstUse)node).Name, node.Position);
            }
            ...
            /// </remarks>
            public override PhpTypeCode Emit(ConstantUse node, CodeGenerator codeGenerator)
            {
                ...
                {
                    var fallbackName = ((GlobalConstUse)node).FallbackName;
        ...
        class ClassConstUseCompiler : ConstantUseCompiler
        {
            ...
            public override Evaluation EvaluatePriorAnalysis(ConstantUse node, SourceUnit sourceUnit)
            {
                var className = ((ClassConstUse)node).ClassName;
                if (!string.IsNullOrEmpty(className.QualifiedName.Name.Value))
                    constant = sourceUnit.TryResolveClassConstantGlobally(className, ((ClassConstUse)node).Name);
            ...
            internal override void ResolveName(ConstantUse node, Analyzer analyzer)
            {
                var classconstuse = (ClassConstUse)node;
                var typeRef = classconstuse.TypeRef;
                ...
                constant = analyzer.ResolveClassConstantName(type, classconstuse.Name, node.Position, analyzer.CurrentType, analyzer.CurrentRoutine,
        ...
        sealed class PseudoClassConstUseCompiler : ClassConstUseCompiler
        {
            internal override void ResolveName(ConstantUse node, Analyzer analyzer)
            {
                if (this.type != null)
            ...
            public override Evaluation Analyze(ConstantUse node, Analyzer analyzer, ExInfoFromParent info)
            {
                ...
                //
                this.ResolveName((ClassConstUse)node, analyzer);
            ...
            public override PhpTypeCode Emit(ConstantUse node, CodeGenerator codeGenerator)
            {
                switch (((PseudoClassConstUse)node).Type)
                {
