    ...
    partial class NodeCompilers
    {
        ...
        sealed class EvalExCompiler : ExpressionCompiler<EvalEx>
        {
            ...
            public override Evaluation Analyze(EvalEx node, Analyzer analyzer, ExInfoFromParent info)
            {
                ...
                if (node.Kind != EvalKinds.SyntheticEval)
                {
            ...
            public override PhpTypeCode Emit(EvalEx node, CodeGenerator codeGenerator)
            {
                ...
                Debug.Assert(node.Kind != EvalKinds.LambdaFunction, "Invalid eval kind.");
                Debug.Assert(node.Kind != EvalKinds.Assert || codeGenerator.Context.Config.Compiler.Debug, "Assert should be cut off in release mode.");
                ...
                if (inlinedCode != null)
                {
                    Debug.Assert(node.Kind == EvalKinds.Assert, "Only assert can be inlined so far.");
                    Label endif_label = il.DefineLabel();
                    Label else_label = il.DefineLabel();
                ...
                {
                    result = codeGenerator.EmitEval(node.Kind, node.Code, node.Position, node.CurrentNamespace, node.CurrentAliases);
                }
