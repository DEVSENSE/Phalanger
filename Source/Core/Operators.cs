...
namespace PHP.Core
{
	...
	public static class Operators
	{
		...
		public static object Add(object x, int y)
		{
			...
			if ((info_x & Convert.NumberInfo.Unconvertible) != 0)
			{
				PhpException.Throw(PhpError.Error, CoreResources.GetString("unsupported_operand_types"));
				return 0;
			}
		...
		public static double Add(object x, double y)
		{
			...
			// converts x to a number:
			if ((Convert.ObjectToNumber(x, out ix, out lx, out dx) & Convert.NumberInfo.Unconvertible) != 0)
			{
				PhpException.Throw(PhpError.Error, CoreResources.GetString("unsupported_operand_types"));
				return 0;
			}
		...
		public static object Subtract(int x, object y)
		{
			...
			if ((info_y & Convert.NumberInfo.Unconvertible) != 0)
			{
				PhpException.Throw(PhpError.Error, CoreResources.GetString("unsupported_operand_types"));
				return 0;
			}
		...
		public static double Subtract(double x, object y)
		{
			...
			// converts x to a number:
			if ((Convert.ObjectToNumber(y, out iy, out ly, out dy) & Convert.NumberInfo.Unconvertible) != 0)
			{
				PhpException.Throw(PhpError.Error, CoreResources.GetString("unsupported_operand_types"));
				return 0;
			}
		...
		public static object Minus(object x)
		{
			...
			switch (Convert.ObjectToNumber(x, out ix, out lx, out dx) & Convert.NumberInfo.TypeMask)
			{
		...
		public static object Divide(double x, object y)
		{
			...
			Convert.ObjectToNumber(y, out iy, out ly, out dy);
		...
		public static object Concat(object x, object y)
		{
			...
			string sx, sy;
			if ((sx = x as string) != null && (sy = y as string) != null) return String.Concat(sx, sy);

            PhpBytes bx,by;
            if ((bx = x as PhpBytes) != null)
            {
                if ((by = y as PhpBytes) != null)
                    return PhpBytes.Concat(bx,by);

                // bytes.object
                return PhpBytes.Concat(bx, Convert.ObjectToPhpBytes(y));
            }
            else if ((by = y as PhpBytes) != null)
            {
                // object.bytes
                return PhpBytes.Concat(Convert.ObjectToPhpBytes(x),by);
            }
            else
            {
                // object.object:
        ...
        public static object Concat(object x, string y)
        {
            ...
            PhpBytes px;
            if ((px = x as PhpBytes) != null)
                return PhpBytes.Concat(px, new PhpBytes(y));
            else
		...
		public static object Concat(string x, object y)
		{
			Debug.Assert(!(y is PhpReference));
            PhpBytes py;
            if ((py = y as PhpBytes) != null)
                return PhpBytes.Concat(new PhpBytes(x), py);
            else
		...
		public static object Append(object x, object y)
		{
            ...
            else if ((sx = x as PhpString) != null)
			{
				// builder.string:
				return sx.Append(Convert.ObjectToString(y));
			}
		...
		public static object Append(object x, string y)
		{
            ...
            {
                return PhpBytes.Concat(bx, new PhpBytes(y));
            }
            else if (ReferenceEquals(x,null))
            {
                return y;
		...
		public static object GetItem(object var, object key, GetItemKinds kind)
		{
			...
			if ((s = var as string) != null)
				return (CheckStringIndexRange(index = Convert.ObjectToInteger(key), s.Length, quiet)) ? s[index].ToString() : null;

			// a character of a PhpString:
			if ((phps = var as PhpString) != null)
				return (CheckStringIndexRange(index = Convert.ObjectToInteger(key), phps.Length, quiet)) ? phps.GetCharUnchecked(index).ToString() : null;
		...
		public static object GetItem(object var, int key, GetItemKinds kind)
		{
			...
			// a character of a string:
			if ((s = var as string) != null)
				return (CheckStringIndexRange(key, s.Length, quiet)) ? s[key].ToString() : null;

			// a character of a PhpString:
			if ((phps = var as PhpString) != null)
				return (CheckStringIndexRange(key, phps.Length, quiet)) ? phps.GetCharUnchecked(key).ToString() : null;
		...
		public static object GetItem(object var, string/*!*/ key, GetItemKinds kind)
		{
			...
			PhpArray array;
			if ((array = var as PhpArray) != null)
				return array.GetArrayItem(key, kind != GetItemKinds.Get);
		...
		public static object GetItemExact(object var, string/*!*/ key, GetItemKinds kind, int hashcode)
		{
			...
			PhpArray array;
			if ((array = var as PhpArray) != null)
				return array.GetArrayItemExact(key, kind != GetItemKinds.Get, hashcode);
		...
		private static object GetStringItemEpilogue(object var, string key, GetItemKinds kind)
		{
			...
			// a character of a string:
			if ((s = var as string) != null)
				return (CheckStringIndexRange(index = Convert.StringToInteger(key), s.Length, quiet)) ? s[index].ToString() : null;

			// a character of a PhpString:
			if ((phps = var as PhpString) != null)
				return (CheckStringIndexRange(index = Convert.StringToInteger(key), phps.Length, quiet)) ? phps.GetCharUnchecked(index).ToString() : null;
		...
		private static object GetItemEpilogue(object var, object key, GetItemKinds kind)
		{
			...
			{
                /* silently returns null, see PHP specs, issue 22019 *///if (!quiet) PhpException.Throw(PhpError.Notice, CoreResources.GetString("empty_used_as_array"));
            ...
            /* silently returns null, see PHP specs, issue 22019 *///PhpException.VariableMisusedAsArray(var, false);
		...
		public static PhpReference GetItemRef(ref object var)
		{
			...
			if (IsEmptyForEnsure(var))
			{
				...
				array.Add(result = new PhpReference());
				return result;
			}
		...
		public static void SetItem(object value, object key, ref object var)
		{
			...
			// string:
			if ((s = var as string) != null && s != String.Empty)
			{
				if (CheckStringIndexRange(index = Convert.ObjectToInteger(key), Int32.MaxValue, false))
					var = SetStringItem(new PhpString(s), index, value);
				return;
			}

			// string builder:
			if ((phps = var as PhpString) != null && phps.Length != 0)
			{
				if (CheckStringIndexRange(index = Convert.ObjectToInteger(key), Int32.MaxValue, false))
					SetStringItem(phps, index, value);
				return;
			}
		...
		public static void SetItem(object value, int key, ref object var)
		{
			...
			if ((s = var as string) != null && s != String.Empty)
			{
				if (CheckStringIndexRange(key, Int32.MaxValue, false))
					var = SetStringItem(new PhpString(s), key, value);
				return;
			}
		...
		public static void SetItemExact(object value, string key, ref object var, int hashcode)
		{
			...
			PhpArray array;
			if ((array = var as PhpArray) != null)
				array.SetArrayItemExact(key, value, hashcode);
			else
		...
		public static PhpArray EnsureVariableIsArray(ref object var)
		{
			...
			// immutable string:
			string str_var = var as string;
			if (str_var != null)
			{
				PhpString phps = new PhpString(str_var);
				var = phps;
				return new PhpArrayString(phps);
			}

			// mutable string:
			if (var is PhpString || var is PhpBytes)
		...
		public static bool StrictEquality(object x, object y)
		{
			...
			if (xtype == typeof(PhpBytes) || xtype == typeof(PhpString)) xtype = typeof(string);
			if (ytype == typeof(PhpBytes) || ytype == typeof(PhpString)) ytype = typeof(string);
