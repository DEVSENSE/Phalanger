...
namespace PHP.Library.Memcached
{
	...
	/// </summary>
    public sealed class MemcachedLibraryDescriptor : PhpLibraryDescriptor
	{
		/// <summary>
		/// Stores one and only instance of the class that is created when the assembly is loaded.
		/// </summary>
        internal static MemcachedLibraryDescriptor Singleton { get { return singleton; } }
        private static MemcachedLibraryDescriptor singleton;
        ...
        //}
        //internal const string ExtensionName = "memcached";
		...
		protected override void Loaded(PhpLibraryAttribute assemblyAttribute, LibraryConfigStore configStore)
		{
			...
			singleton = this;
			MemcachedConfiguration.RegisterLegacyOptions();
		}
		...
		{
			// parses XML tree:
			ConfigUtils.ParseNameValueList(section, context, (MemcachedLocalConfig)result.Local, (MemcachedGlobalConfig)result.Global);
		...
		{
			return new ConfigContextBase(new MemcachedLocalConfig(), new MemcachedGlobalConfig());
		}
