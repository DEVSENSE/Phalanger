...
namespace Enyim.Caching.Memcached.Operations.Text
{
	internal sealed class IncrementOperation : ItemOperation
	{
        ...
        internal IncrementOperation(IServerPool pool, string key, string serverKey, ulong delta)
		...
		protected override ResConstants ExecuteAction()
		{
			...
			TextSocketHelper.SendCommand(socket, String.Concat("incr ", this.HashedKey, " ", this.delta.ToString(CultureInfo.InvariantCulture)));
