...
namespace Enyim.Caching.Memcached.Operations.Text
{
	internal static class GetHelper
	{
        ...
        public static ResConstants HandleResponse(string response)
        {
            if (response == null)
            ...
            if (String.Compare(args[0], "CLIENT_ERROR", StringComparison.Ordinal) == 0)
                return ResConstants.ClientError;
            if (String.Compare(args[0], "SERVER_ERROR ", StringComparison.Ordinal) == 0)
