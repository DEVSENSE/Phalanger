	...
	public sealed class DefaultNodeLocator : IMemcachedNodeLocator
	{
        ...
        void IMemcachedNodeLocator.Initialize(ICollection<IMemcachedNode> nodes, HashAlgorithm hash)
		{
			...
			lock (this.initLock)
			{
				...
				{
					List<uint> tmpKeys = GenerateKeys(node, ServerAddressMutations, hash);
		...
		private static List<uint> GenerateKeys(IMemcachedNode node, int numberOfKeys, HashAlgorithm hash)
		{
			...
			{
                byte[] data = hash.ComputeHash(Encoding.ASCII.GetBytes(String.Concat(address, "-", i)));
