...
//
// RegExpressionEngine.cpp 
// - slightly modified engine.c, originally PHP source file
// - excluded from build (included from RegExpressions.cpp)
//
...
dissect(register struct match *m, char *start, char *stop, sopno startst, sopno stopst)
{
	...
	for (ss = startst; ss < stopst; ss = es) {
		...
		case OLPAREN:
			i = OPND(m->g->strip[ss]);
			assert(0 < i && (unsigned)i <= m->g->nsub);
...
backref(register struct match *m, char *start, char *stop, sopno startst, sopno stopst, sopno lev)
{
	...
	case OBACK_:		/* the vilest depths */
		...
		assert((unsigned)(stop - m->beginp) >= len);
		if (sp > stop - len)
