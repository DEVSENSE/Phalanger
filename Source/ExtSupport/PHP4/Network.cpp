...
//
// Network.cpp 
// - slightly modified network.c, originally PHP source file
//
...
/*
   ...
   +----------------------------------------------------------------------+
 */
/* $Id: Network.cpp,v 1.1.2.2 2006/04/15 20:19:12 prosl0am Exp $ */
...
# endif
	...
	*sal = (sockaddr **)emalloc(2 * sizeof(*sal));
	sap = *sal;
	*sap = (sockaddr *)emalloc(sizeof(struct sockaddr_in));
...
/* {{{ php_connect_nonb */
ZEND_API int php_connect_nonb(int sockfd,
...
ZEND_API int php_connect_nonb_win32(SOCKET sockfd,
						const struct sockaddr *addr,
						socklen_t addrlen,
						struct timeval *timeout)
{
 ...
 */
ZEND_API int php_hostconnect(const char *host, unsigned short port, int socktype, struct timeval *timeout TSRMLS_DC)
{	
	int n, repeatto, s;
 ...
 */
ZEND_API void php_any_addr(int family, php_sockaddr_storage *addr, unsigned short port)
{
 ...
 */
ZEND_API int php_sockaddr_size(php_sockaddr_storage *addr)
{
...
ZEND_API char *php_socket_strerror(long err, char *buf, size_t bufsize)
{
#ifndef PHP_WIN32
	char *errstr;
...
ZEND_API php_stream *_php_stream_sock_open_from_socket(int socket, const char *persistent_id STREAMS_DC TSRMLS_DC)
{
	php_stream *stream;
	php_netstream_data_t *sock;

	sock = (php_netstream_data_t *)pemalloc(sizeof(php_netstream_data_t), persistent_id ? 1 : 0);
	...
	sock->timeout.tv_sec = PHP::Core::ScriptContext::CurrentContext->Config->FileSystem->DefaultSocketTimeout; //FG(default_socket_timeout);
...
ZEND_API php_stream *_php_stream_sock_open_host(const char *host, unsigned short port,
...
ZEND_API php_stream *_php_stream_sock_open_unix(const char *path, int pathlen, const char *persistent_id,
...
#ifdef HAVE_OPENSSL_EXT
ZEND_API int php_stream_sock_ssl_activate_with_method(php_stream *stream, int activate, SSL_METHOD *method, php_stream *session_stream TSRMLS_DC)
{
...
ZEND_API int php_set_sock_blocking(int socketd, int block TSRMLS_DC)
{
      int ret = SUCCESS;
      int flags;
      int myflag = 0;
...
{
	php_netstream_data_t *sock = (php_netstream_data_t*)stream->abstract;
	return crtx_fstat(sock->socket, &ssb->sb);
}
...
#endif
			if (ret) {
				*(FILE**)ret = crtx_fdopen(sock->socket, stream->mode);
				if (*ret) {
					return SUCCESS;
				}
				return FAILURE;
