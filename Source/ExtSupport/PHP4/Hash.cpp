...
//
// Hash.cpp 
// - contains definitions of hash tables (= PHP arrays) related functions
// - this is slightly modified zend_hash.c and zend_ts_hash.c of php-4.3.2 sources
//
...
/* $Id: Hash.cpp,v 1.1.2.2 2006/04/15 20:19:12 prosl0am Exp $ */
...
// LP: for PHP 4 compatibility
#ifdef zend_hash_init
#define _temp zend_hash_init
...
{
	return _zend_hash_init(ht, nSize, pHashFunction, pDestructor, persistent);
}
#ifdef _temp
#define zend_hash_init _temp
#undef _temp
#endif

// LP: for PHP 4 compatibility
#ifdef zend_hash_init_ex
#define _temp zend_hash_init_ex
...
#ifdef _temp
#define zend_hash_init_ex _temp
#undef _temp
#endif
...
{
	return zend_hash_add_or_update(ht, arKey, nKeyLength, pData, nDataSize, pDest, flag);
}
...
// LP: for PHP 5 compatibility
ZEND_API int _zend_hash_quick_add_or_update(HashTable *ht, char *arKey, uint nKeyLength, ulong h, void *pData, uint nDataSize, void **pDest, int flag)
{
	return zend_hash_quick_add_or_update(ht, arKey, nKeyLength, h, pData, nDataSize, pDest, flag);
}
...
{
	return zend_hash_index_update_or_next_insert(ht, h, pData, nDataSize, pDest, flag);
}
...
{
	zend_hash_merge(target, source, pCopyConstructor, tmp, size, overwrite);
}
...
{
	// some extensions (GTK) work directly with EG(function_table) and EG(class_table)
	// the following lines attempt to support it 8-}
	TSRMLS_FETCH();
	if (ht == EG(class_table))
	{
		*pData = &zend_standard_class_def;
		return SUCCESS;
	}
...
ZEND_API int zend_hash_exists(HashTable *ht, char *arKey, uint nKeyLength)
{
	...
	// the following lines attempt to support it 8-}
	TSRMLS_FETCH();
	if (ht == EG(function_table))
	{
/*
		PhpCallback ^callback = gcnew PhpCallback(gcnew String(arKey, 0, nKeyLength - 1));
		Request ^request = Request::GetCurrentRequest();

		RequestCookie ^cookie = request->GetCookie();
		if (cookie != NULL)
		{
			return cookie->CheckCallback(callback);
		}
		else
		{
			return RequestCookie::CheckCallbackDirect(callback);
		}
*/
		return 1;
	}
	if (ht == &zend_standard_class_def.function_table || PHP::ExtManager::Module::GetCurrentModule()->IsNativeMethodTable(ht))
	{
		return 1;
	}
