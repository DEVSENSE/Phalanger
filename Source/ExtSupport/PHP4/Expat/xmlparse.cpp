...
static int
defineAttribute(ELEMENT_TYPE_x *type, ATTRIBUTE_ID *,
...
getAttributeId(XML_Parser parser, const ENCODING *enc, const char *start,
               const char *end);
static int
setElementTypePrefix(XML_Parser parser, ELEMENT_TYPE_x *);
static enum XML_Error
...
struct XML_ParserStruct {
  ...
  ELEMENT_TYPE_x *m_declElementType;
                               ...
                               const XML_Char *encodingName)
{
  ...
  ELEMENT_TYPE_x * oldDeclElementType = declElementType;
          ...
          BINDING **bindingsPtr)
{
  ...
  elementType = (ELEMENT_TYPE_x *)lookup(&dtd->elementTypes, tagNamePtr->str,0);
  if (!elementType) {
    const XML_Char *name = poolCopyString(&dtd->pool, tagNamePtr->str);
    if (!name)
      return XML_ERROR_NO_MEMORY;
    elementType = (ELEMENT_TYPE_x *)lookup(&dtd->elementTypes, name,
                                         sizeof(ELEMENT_TYPE_x));
    if (!elementType)
      return XML_ERROR_NO_MEMORY;
...
static int
defineAttribute(ELEMENT_TYPE_x *type, ATTRIBUTE_ID *attId, XML_Bool isCdata,
...
static int
setElementTypePrefix(XML_Parser parser, ELEMENT_TYPE_x *elementType)
{
  DTD * const dtd = _dtd;  /* save one level of indirection */
  const XML_Char *name;
...
dtdReset(DTD *p, const XML_Memory_Handling_Suite *ms)
{
  ...
  for (;;) {
    ELEMENT_TYPE_x *e = (ELEMENT_TYPE_x *)hashTableIterNext(&iter);
    if (!e)
      break;
...
dtdCopy(DTD *newDtd, const DTD *oldDtd, const XML_Memory_Handling_Suite *ms)
{
  ...
  for (;;) {
    ...
    ELEMENT_TYPE_x *newE;
    const XML_Char *name;
    const ELEMENT_TYPE_x *oldE = (ELEMENT_TYPE_x *)hashTableIterNext(&iter);
    if (!oldE)
      break;
    name = poolCopyString(&(newDtd->pool), oldE->name);
    if (!name)
      return 0;
    newE = (ELEMENT_TYPE_x *)lookup(&(newDtd->elementTypes), name,
                                  sizeof(ELEMENT_TYPE_x));
    if (!newE)
      return 0;
               ...
               const char *end)
{
  ...
  if (!name)
    return NULL;
  ret = (ELEMENT_TYPE_x *) lookup(&dtd->elementTypes, name, sizeof(ELEMENT_TYPE_x));
  if (!ret)
    return NULL;
  if (ret->name != name)
    poolDiscard(&dtd->pool);
