...
//
// StreamMemory.cpp 
// - slightly modified memory.c, originally PHP source file
//
...
/* $Id: StreamMemory.cpp,v 1.1.2.2 2006/04/15 20:19:12 prosl0am Exp $ */
...
static size_t php_stream_memory_write(php_stream *stream, const char *buf, size_t count TSRMLS_DC)
{
	...
	if (ms->fpos + count > ms->fsize) {
		...
		if (!ms->data) {
			tmp = (char *)emalloc(ms->fpos + count);
		} else {
			tmp = (char *)erealloc(ms->data, ms->fpos + count);
		}
		if (!tmp) {
...
ZEND_API php_stream *_php_stream_memory_create(int mode STREAMS_DC TSRMLS_DC)
{
	...
	self = (php_stream_memory_data *)emalloc(sizeof(*self));
	self->data = NULL;
	self->fpos = 0;
	self->fsize = 0;
	self->smax = -1;
...
ZEND_API php_stream *_php_stream_temp_create(int mode, size_t max_memory_usage STREAMS_DC TSRMLS_DC)
{
	...
	self = (php_stream_temp_data *)ecalloc(1, sizeof(*self));
...
ZEND_API php_stream *_php_stream_temp_open(int mode, size_t max_memory_usage, char *buf, size_t length STREAMS_DC TSRMLS_DC)
{
	...
	if ((stream = php_stream_temp_create_rel(mode & ~TEMP_STREAM_READONLY, max_memory_usage)) != NULL) {
		...
		ms = (php_stream_temp_data *)stream->abstract;
		assert(ms != NULL);
		ms->mode = mode;
	}
	return stream;
