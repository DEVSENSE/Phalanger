...
//
// VirtualWorkingDir.cpp 
// - slightly modified tsrm_virtual_cwd.c, originally PHP source file
//
...
{
	char *dir = PHP::ExtManager::PhpMarshaler::MarshalManagedStringToNativeStringPersistent(
		/*PHP::Library::PhpDirectory::GetWorking()*/vwdir);
	if (dir != NULL)
	{
		TSRMLS_FETCH();

		if (CWDG(cwd).cwd != NULL) free(CWDG(cwd).cwd);
		CWDG(cwd).cwd = dir;
		CWDG(cwd).cwd_length = strlen(dir);
	}
}
...
/* $Id: VirtualWorkingDir.cpp,v 1.1.2.2 2006/04/15 20:19:12 prosl0am Exp $ */
...
ZEND_API void virtual_cwd_startup(void)
{
	...
	main_cwd_state.cwd = _strdup(cwd);
...
ZEND_API FILE *virtual_fopen(const char *path, const char *mode TSRMLS_DC)
{
	...
	f = crtx_fopen(new_state.cwd, mode);
...
#pragma unmanaged	// varargs are not supported in /clr
ZEND_API int virtual_open(const char *path TSRMLS_DC, int flags, ...)
{
	...
	if (flags & O_CREAT) {
		...
		f = crtx_open(new_state.cwd, flags, mode);
	} else {
		f = crtx_open(new_state.cwd, flags, 0);
	}	
	CWD_STATE_FREE(&new_state);
	return f;
}
...
ZEND_API int virtual_creat(const char *path, mode_t mode TSRMLS_DC)
{
	...
	f = crtx_creat(new_state.cwd,  mode);
