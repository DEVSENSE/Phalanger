...
//
// Scanf.cpp
// - slightly modified scanf.c, originally PHP source file
//
...
/* $Id: Scanf.cpp,v 1.1.2.2 2006/04/15 20:19:12 prosl0am Exp $ */
 ...
 *----------------------------------------------------------------------
*/
ZEND_API int ValidateFormat(char *format, int numVars, int *totalSubs)
{
	...
	if (!(flags & SCAN_SUPPRESS)) {
	    if (objIndex >= nspace) {
            ...
            if (nassign == staticAssign) {
                nassign = (int *)safe_emalloc(nspace, sizeof(int), 0);
                for (i = 0; i < STATIC_LIST_SIZE; ++i) {
                    nassign[i] = staticAssign[i];
                }
            } else {
                nassign = (int *)erealloc((void *)nassign, nspace * sizeof(int));
            }
...
ZEND_API int php_sscanf_internal(	char *string, char *format,
