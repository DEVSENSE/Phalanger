...
//
// Streams.cpp 
// - slightly modified streams.c and user_streams.c, originally PHP source file
//
...
/* $Id: Streams.cpp,v 1.1.2.2 2006/04/15 20:19:12 prosl0am Exp $ */
...
#define PHP_STDIOP_GET_FD(anfd, data)   anfd = (data)->file ? crtx_fileno((data)->file) : (data)->fd
...
ZEND_API int php_file_le_stream(void)
{
	return le_stream;
}

ZEND_API int php_file_le_pstream(void)
{
	return le_pstream;
}
...
void request_shutdown_streams(TSRMLS_D)
{
	zend_hash_apply(&EG(persistent_list), (apply_func_t)forget_persistent_resource_id_numbers TSRMLS_CC);
}

ZEND_API int php_stream_from_persistent_id(const char *persistent_id, php_stream **stream TSRMLS_DC)
{
...
ZEND_API php_stream *_php_stream_alloc(php_stream_ops *ops, void *abstract, const char *persistent_id, const char *mode STREAMS_DC TSRMLS_DC) /* {{{ */
{
	php_stream *ret;
...
ZEND_API int _php_stream_free(php_stream *stream, int close_options TSRMLS_DC) /* {{{ */
{
	int ret = 1;
	int remove_rsrc = 1;
...
fprintf(stderr, "stream_free: %s:%p[%s] preserve_handle=%d release_cast=%d remove_rsrc=%d\n",
	...
	if (close_options & PHP_STREAM_FREE_CALL_DTOR) {
		if (release_cast && stream->fclose_stdiocast == PHP_STREAM_FCLOSE_FOPENCOOKIE) {
			 ...
			 */
			stream->in_free = 0;
			return crtx_fclose(stream->stdiocast);
		}
...
ZEND_API int php_stream_filter_register_factory(const char *filterpattern, php_stream_filter_factory *factory TSRMLS_DC)
{
	return zend_hash_add(&stream_filters_hash, (char*)filterpattern, strlen(filterpattern), factory, sizeof(*factory), NULL);
}

ZEND_API int php_stream_filter_unregister_factory(const char *filterpattern TSRMLS_DC)
{
...
ZEND_API php_stream_filter *php_stream_filter_create(const char *filtername, const char *filterparams, int filterparamslen, int persistent TSRMLS_DC)
{
...
ZEND_API php_stream_filter *_php_stream_filter_alloc(php_stream_filter_ops *fops, void *abstract, int persistent STREAMS_DC TSRMLS_DC)
{
...
ZEND_API void php_stream_filter_free(php_stream_filter *filter TSRMLS_DC)
{
	if (filter->fops->dtor)
...
ZEND_API void php_stream_filter_prepend(php_stream *stream, php_stream_filter *filter)
{
...
ZEND_API void php_stream_filter_append(php_stream *stream, php_stream_filter *filter)
{
...
ZEND_API php_stream_filter *php_stream_filter_remove(php_stream *stream, php_stream_filter *filter, int call_dtor TSRMLS_DC)
{
...
static void php_stream_fill_read_buffer(php_stream *stream, size_t size TSRMLS_DC)
{
	...
	if (stream->writepos - stream->readpos < (off_t)size) {
		...
		if (stream->readbuflen - stream->writepos < stream->chunk_size) {
			...
			stream->readbuf = (unsigned char *)perealloc(stream->readbuf, stream->readbuflen,
...
ZEND_API size_t _php_stream_read(php_stream *stream, char *buf, size_t size TSRMLS_DC)
{
...
ZEND_API int _php_stream_eof(php_stream *stream TSRMLS_DC)
{
...
ZEND_API int _php_stream_putc(php_stream *stream, int c TSRMLS_DC)
{
	unsigned char buf = c;
...
ZEND_API int _php_stream_getc(php_stream *stream TSRMLS_DC)
{
	unsigned char buf;

	if (php_stream_read(stream, (char *)&buf, 1) > 0) {
		return buf & 0xff;
	}
	return EOF;
}

ZEND_API int _php_stream_puts(php_stream *stream, char *buf TSRMLS_DC)
{
	int len;
	char newline[2] = "\n"; /* is this OK for Win? */
	len = strlen(buf);
...
ZEND_API int _php_stream_stat(php_stream *stream, php_stream_statbuf *ssb TSRMLS_DC)
{
...
ZEND_API char *php_stream_locate_eol(php_stream *stream, char *buf, size_t buf_len TSRMLS_DC)
{
	size_t avail;
	char *cr, *lf, *eol = NULL;
	char *readptr;
	...
	if (stream->flags & PHP_STREAM_FLAG_DETECT_EOL) {
		cr = (char *)memchr(readptr, '\r', avail);
		lf = (char *)memchr(readptr, '\n', avail);
		...
		eol = (char *)memchr(readptr, '\r', avail);
	} else {
		/* unix (and dos) line endings */
		eol = (char *)memchr(readptr, '\n', avail);
	}
 ...
 * permitting */
ZEND_API char *_php_stream_get_line(php_stream *stream, char *buf, size_t maxlen,
	...
	for (;;) {
		...
		if (avail > 0) {
			...
			if (grow_mode) {
				 ...
				 * hard to follow */
				bufstart = (char *)erealloc(bufstart, current_buf_size + cpysz + 1);
...
ZEND_API int _php_stream_flush(php_stream *stream, int closing TSRMLS_DC)
{
	int ret = 0;
...
ZEND_API size_t _php_stream_write(php_stream *stream, const char *buf, size_t count TSRMLS_DC)
{
...
ZEND_API size_t _php_stream_printf(php_stream *stream TSRMLS_DC, const char *fmt, ...)
{
	size_t count;
	char *buf;
	va_list ap;
...
ZEND_API off_t _php_stream_tell(php_stream *stream TSRMLS_DC)
{
	return stream->position;
}

ZEND_API int _php_stream_seek(php_stream *stream, off_t offset, int whence TSRMLS_DC)
{
...
ZEND_API int _php_stream_set_option(php_stream *stream, int option, int value, void *ptrparam TSRMLS_DC)
{
...
ZEND_API size_t _php_stream_passthru(php_stream * stream STREAMS_DC TSRMLS_DC)
{
	size_t bcount = 0;
	int ready = 0;
	char buf[8192];
...
ZEND_API size_t _php_stream_copy_to_mem(php_stream *src, char **buf, size_t maxlen, int persistent STREAMS_DC TSRMLS_DC)
{
	size_t ret = 0;
	char *ptr;
	...
	ptr = *buf = (char *)pemalloc_rel_orig(max_len, persistent);

	while((ret = php_stream_read(src, ptr, max_len - len)))	{
		len += ret;
		if (len + min_room >= max_len) {
			*buf = (char *)perealloc_rel_orig(*buf, max_len + step, persistent);
			max_len += step;
			ptr = *buf + len;
		} else {
			ptr += ret;
	...
	if (len) {
		*buf = (char *)perealloc_rel_orig(*buf, len + 1, persistent);
		(*buf)[len] = '\0';
	} else {
...
ZEND_API size_t _php_stream_copy_to_stream(php_stream *src, php_stream *dest, size_t maxlen STREAMS_DC TSRMLS_DC)
{
	char buf[CHUNK_SIZE];
	size_t readchunk;
	size_t haveread = 0;
...
ZEND_API php_stream *_php_stream_fopen_temporary_file(const char *dir, const char *pfx, char **opened_path STREAMS_DC TSRMLS_DC)
{
...
ZEND_API php_stream *_php_stream_fopen_tmpfile(int dummy STREAMS_DC TSRMLS_DC)
{
...
ZEND_API php_stream *_php_stream_fopen_from_file(FILE *file, const char *mode STREAMS_DC TSRMLS_DC)
{
	php_stdio_stream_data *self;
	php_stream *stream;
	
	self = (php_stdio_stream_data *)emalloc_rel_orig(sizeof(*self));
	self->file = file;
	self->is_pipe = 0;
	self->is_process_pipe = 0;
	self->temp_file_name = NULL;
	self->fd = crtx_fileno(file);

#ifdef S_ISFIFO
	/* detect if this is a pipe */
	if (self->fd >= 0) {
		struct stat sb;
		self->is_pipe = (crtx_fstat(self->fd, &sb) == 0 && S_ISFIFO(sb.st_mode)) ? 1 : 0;
	}
#endif
	...
	if (stream) {
		if (self->is_pipe) {
			...
			stream->position = crtx_ftell(file);
		}
	}
...
ZEND_API php_stream *_php_stream_fopen_from_pipe(FILE *file, const char *mode STREAMS_DC TSRMLS_DC)
{
	php_stdio_stream_data *self;
	php_stream *stream;
...
static size_t php_stdiop_write(php_stream *stream, const char *buf, size_t count TSRMLS_DC)
{
	...
	if (data->fd >= 0) {
		int bytes_written = crtx_write(data->fd, buf, count);
		...
		if (!data->is_pipe && data->last_op == 'r') {
			crtx_fseek(data->file, 0, SEEK_CUR);
		}
		data->last_op = 'w';
#endif

		return crtx_fwrite(buf, 1, count, data->file);
	}
}
...
static size_t php_stdiop_read(php_stream *stream, char *buf, size_t count TSRMLS_DC)
{
	...
	if (data->fd >= 0) {
		ret = crtx_read(data->fd, buf, count);
		...
		ret = crtx_fread(buf, 1, count, data->file);
	...
	if (close_handle) {
		if (data->file) {
			if (data->is_process_pipe) {
				...
				ret = crtx_fclose(data->file);
			}
			data->file = NULL;
			data->fd = -1;
		} else if (data->fd != -1) {
			ret = crtx_close(data->fd);
			data->fd = -1;
		} else {
			return 0;/* everything should be closed already -> success*/
		}
		if (data->temp_file_name) {
			_unlink(data->temp_file_name);
...
static int php_stdiop_seek(php_stream *stream, off_t offset, int whence, off_t *newoffset TSRMLS_DC)
{
	...
	if (data->fd >= 0) {
		...
		result = crtx_lseek(data->fd, offset, whence);
		if (result == (off_t)-1)
			return -1;
		...
		*newoffset = result;
		...
		ret = crtx_fseek(data->file, offset, whence);
		*newoffset = crtx_ftell(data->file);
		return ret;
	}
}
...
static int php_stdiop_cast(php_stream *stream, int castas, void **ret TSRMLS_DC)
{
	...
	switch (castas)	{
		case PHP_STREAM_AS_STDIO:
			if (ret) {
				if (data->file == NULL) {
					data->file = crtx_fdopen(data->fd, stream->mode);
				}
				*(FILE**)ret = data->file;
				data->fd = -1;
			}
		...
		case PHP_STREAM_OPTION_WRITE_BUFFER:
			...
			switch(value) {
				case PHP_STREAM_BUFFER_NONE:
					stream->flags |= PHP_STREAM_FLAG_NO_BUFFER;
					return crtx_setvbuf(data->file, NULL, _IONBF, 0);
					
				case PHP_STREAM_BUFFER_LINE:
					stream->flags ^= PHP_STREAM_FLAG_NO_BUFFER;
					return crtx_setvbuf(data->file, NULL, _IOLBF, size);
					
				case PHP_STREAM_BUFFER_FULL:
					stream->flags ^= PHP_STREAM_FLAG_NO_BUFFER;
					return crtx_setvbuf(data->file, NULL, _IOFBF, size);
...
ZEND_API php_stream_ops	php_stream_stdio_ops = {
...
ZEND_API php_stream *_php_stream_fopen_with_path(char *filename, char *mode, char *path, char **opened_path, int options STREAMS_DC TSRMLS_DC)
{
...
not_relative_path:
	...
	if (IS_SLASH(filename[0])) {
		int cwd_len;
		char *cwd;
		cwd = virtual_getcwd_ex((size_t *)&cwd_len TSRMLS_CC);
...
ZEND_API int php_stream_parse_fopen_modes(const char *mode, int *open_flags)
{
	int flags;
...
/* {{{ php_stream_fopen */
ZEND_API php_stream *_php_stream_fopen(const char *filename, const char *mode, char **opened_path, int options STREAMS_DC TSRMLS_DC)
{
	char *realpath = NULL;
	struct stat st;
	int open_flags;
	...
	fd = crtx_open(realpath, open_flags, 0666);

	if (fd != -1)	{
		/* sanity checks for include/require */
		if (options & STREAM_OPEN_FOR_INCLUDE && (crtx_fstat(fd, &st) == -1 || !S_ISREG(st.st_mode))) {
#ifdef PHP_WIN32
...
ZEND_API php_stream *_php_stream_fopen_from_fd(int fd, const char *mode, const char *persistent_id STREAMS_DC TSRMLS_DC)
{
	php_stdio_stream_data *self;
	php_stream *stream;
	...
	stat_ok = fd >= 0 && crtx_fstat(fd, &sb) == 0;
#endif
	...
	self = (php_stdio_stream_data *)pemalloc_rel_orig(sizeof(*self), persistent_id);
	...
	{
		long handle = crtx_get_osfhandle(self->fd);
	...
	if (stream) {
		if (self->is_pipe) {
			...
			stream->position = crtx_lseek(self->fd, 0, SEEK_CUR);
		}
	}
...
/* {{{ php_stream_cast */
ZEND_API int _php_stream_cast(php_stream *stream, int castas, void **ret, int show_err TSRMLS_DC)
{
...
ZEND_API int php_register_url_stream_wrapper(char *protocol, php_stream_wrapper *wrapper TSRMLS_DC)
{
#ifdef DEBUG
	Debug::WriteLine("PHP4TS", String::Concat("Registering stream wrapper for protocol: ", gcnew String(protocol)));
#endif

	if (zend_hash_add(&url_stream_wrappers_hash, protocol, strlen(protocol), wrapper, sizeof(*wrapper), NULL) == SUCCESS)
	{
		PHP::ExtManager::StreamWrapperProxy::RegisterWrapper(gcnew String(protocol, 0, strlen(protocol),
			PHP::ExtManager::Request::AppConf->Globalization->PageEncoding));
		return SUCCESS;
	}
	else return FAILURE;
}

ZEND_API int php_unregister_url_stream_wrapper(char *protocol TSRMLS_DC)
{
	if (zend_hash_del(&url_stream_wrappers_hash, protocol, strlen(protocol)) == SUCCESS)
	{
		PHP::ExtManager::StreamWrapperProxy::UnregisterWrapper(gcnew String(protocol, 0, strlen(protocol),
			PHP::ExtManager::Request::AppConf->Globalization->PageEncoding));
		return SUCCESS;
	}
	else return FAILURE;
...
ZEND_API int php_register_url_stream_wrapper_volatile(char *protocol, php_stream_wrapper *wrapper TSRMLS_DC)
{
#ifdef DEBUG
	Debug::WriteLine("PHP4TS", String::Concat("Unsupported: trying to registering volatile stream wrapper for protocol: ",
		gcnew String(protocol)));
#endif
	return FAILURE;
}
...
static int php_plain_files_unlink(php_stream_wrapper *wrapper, char *url, int options, php_stream_context *context TSRMLS_DC)
{
	...
	// TODO: php_clear_stat_cache(1, NULL, 0 TSRMLS_CC);
...
ZEND_API php_stream_wrapper *php_stream_locate_url_wrapper(const char *path, char **path_for_open, int options TSRMLS_DC)
{
...
ZEND_API int _php_stream_stat_path(char *path, php_stream_statbuf *ssb TSRMLS_DC)
{
	php_stream_wrapper *wrapper = NULL;
...
/* {{{ _php_stream_mkdir
*/
ZEND_API int _php_stream_mkdir(char *path, int mode, int options, php_stream_context *context TSRMLS_DC)
{
	php_stream_wrapper *wrapper = NULL;
...
/* {{{ _php_stream_rmdir
*/
ZEND_API int _php_stream_rmdir(char *path, int options, php_stream_context *context TSRMLS_DC)
{
	php_stream_wrapper *wrapper = NULL;
...
/* {{{ php_stream_opendir */
ZEND_API php_stream *_php_stream_opendir(char *path, int options,
...
ZEND_API php_stream_dirent *_php_stream_readdir(php_stream *dirstream, php_stream_dirent *ent TSRMLS_DC)
{
...
ZEND_API int php_stream_dirent_alphasort(const char **a, const char **b)
{
	return strcoll(*a, *b);
}

ZEND_API int php_stream_dirent_alphasortr(const char **a, const char **b)
{
	return strcoll(*b, *a);
}

ZEND_API int _php_stream_scandir(char *dirname, char **namelist[], int flags, php_stream_context *context,
...
ZEND_API void php_stream_wrapper_log_error(php_stream_wrapper *wrapper, int options TSRMLS_DC, const char *fmt, ...)
{
	va_list args;
	char *buffer = NULL;
	...
	if (options & REPORT_ERRORS || wrapper == NULL) {
		...
		/* append to stack */
		wrapper->err_stack = (char **)erealloc(wrapper->err_stack, (wrapper->err_count + 1) * sizeof(char *));
		if (wrapper->err_stack)
...
ZEND_API php_stream *_php_stream_open_wrapper_ex(char *path, char *mode, int options,
...
ZEND_API FILE * _php_stream_open_wrapper_as_file(char *path, char *mode, int options, char **opened_path STREAMS_DC TSRMLS_DC)
{
	FILE *fp = NULL;
	php_stream *stream = NULL;
...
ZEND_API zend_bool _php_stream_open_wrapper_as_file_handle(char *path, char *mode, int options, zend_file_handle *fh STREAMS_DC TSRMLS_DC)
{
	php_stream *stream = NULL;
	int is_sock = 0;
...
ZEND_API int _php_stream_make_seekable(php_stream *origstream, php_stream **newstream, int flags STREAMS_DC TSRMLS_DC)
{
...
ZEND_API php_stream_context *php_stream_context_set(php_stream *stream, php_stream_context *context)
{
...
ZEND_API void php_stream_notification_notify(php_stream_context *context, int notifycode, int severity,
...
ZEND_API void php_stream_context_free(php_stream_context *context)
{
	if (context->options) {
...
ZEND_API php_stream_context *php_stream_context_alloc(void)
{
...
ZEND_API php_stream_notifier *php_stream_notification_alloc(void)
{
	return (php_stream_notifier *)ecalloc(1, sizeof(php_stream_notifier));
}

ZEND_API void php_stream_notification_free(php_stream_notifier *notifier)
{
	if (notifier->dtor) {
...
ZEND_API int php_stream_context_get_option(php_stream_context *context,
...
ZEND_API int php_stream_context_set_option(php_stream_context *context,
		const char *wrappername, const char *optionname, zval *optionvalue)
{
	zval **wrapperhash;
...
ZEND_API HashTable *php_stream_get_url_stream_wrappers_hash()
{
	return &url_stream_wrappers_hash;
}

ZEND_API HashTable *_php_stream_get_url_stream_wrappers_hash()
{
	return &url_stream_wrappers_hash;
}
