...
//
// zend_compile.cpp
// - this is slightly modified zend_compile.c, originally PHP 5.3.3 source files
//
...
{
	TMP_UNSUPPORTED_MSG("zend_auto_global_disable_jit");
	return FAILURE;
...
{
	TMP_UNSUPPORTED_MSG("file_handle_dtor");
...
//		op_array->vars = (zend_compiled_variable*)erealloc(op_array->vars, op_array->size_var*sizeof(zend_compiled_variable));
//	}
...
//			char *class_lcname;
//			
//			class_lcname = (char*)do_alloca(CG(active_class_entry)->name_length + 1, use_heap);
...
//		SET_UNUSED(opline->op2);
//	}
//	CG(active_op_array)->arg_info = (zend_arg_info*)erealloc(CG(active_op_array)->arg_info, sizeof(zend_arg_info)*(CG(active_op_array)->num_args));
...
//	char *lcname;
//	char *is_compound = (char*)memchr(Z_STRVAL(function_name->u.constant), '\\', Z_STRLEN(function_name->u.constant));
//
...
//		Z_STRLEN(opline->op1.u.constant) = Z_STRLEN(opline->op2.u.constant);
//		opline->extended_value = zend_hash_func(Z_STRVAL(opline->op1.u.constant), Z_STRLEN(opline->op1.u.constant) + 1);
//		slash = (char*)zend_memrchr(Z_STRVAL(opline->op1.u.constant), '\\', Z_STRLEN(opline->op1.u.constant));
...
//	int len;
//	zval **ns;
//	char *lcname, *compound = (char*)memchr(Z_STRVAL(element_name->u.constant), '\\', Z_STRLEN(element_name->u.constant));
//
...
//	int len;
//
//	compound = (char*)memchr(Z_STRVAL(class_name->u.constant), '\\', Z_STRLEN(class_name->u.constant));
//	if (compound) {
...
//			Z_STRVAL(class_name->u.constant) = (char*)erealloc(
...
//		result->u.constant.value.str.val = (char*)erealloc(result->u.constant.value.str.val, length+1);
...
//
//	CG(active_op_array)->try_catch_array = (zend_try_catch_element*)erealloc(CG(active_op_array)->try_catch_array, sizeof(zend_try_catch_element)*CG(active_op_array)->last_try_catch);
...
{
	TMP_UNSUPPORTED_MSG("zend_do_inherit_interfaces");
...
/* }}} */
//
//int inherit_static_prop(zval **p TSRMLS_DC, int num_args, va_list args, const zend_hash_key *key) /* {{{ */
//{
...
{
	TMP_UNSUPPORTED_MSG("zend_do_inheritance");
...
{
	TMP_UNSUPPORTED_MSG("zend_do_implement_interface");
...
{
	UNSUPPORTED_MSG("do_bind_function");
	return FAILURE;
...
{
	UNSUPPORTED_MSG("do_bind_inherited_class");
	return NULL;
...
{
	UNSUPPORTED_MSG("zend_do_delayed_early_binding");
...
{
	UNSUPPORTED_MSG("zend_mangle_property_name");
...
ZEND_API int zend_unmangle_property_name(char *mangled_property, int len, char **class_name, char **prop_name) /* {{{ */
{
	...
	UNSUPPORTED_MSG("zend_unmangle_property_name");
	return FAILURE;
...
//	switch (mode) {
//		case ZEND_CT:
//			compound = (char*)memchr(Z_STRVAL(constant_name->u.constant), '\\', Z_STRLEN(constant_name->u.constant));
...
//				Z_STRVAL(offset->u.constant) = (char*)erealloc(Z_STRVAL(offset->u.constant), Z_STRLEN(offset->u.constant)+3);
...
{
	TMP_UNSUPPORTED_MSG("zend_initialize_class_data");
...
//			CG(static_members) = (HashTable**)realloc(CG(static_members), (n+1)*sizeof(HashTable*));
...
//
//	if (!CG(current_import)) {
//		CG(current_import) = (HashTable*)emalloc(sizeof(HashTable));
...
//		char *c_ns_name = (char*)emalloc(Z_STRLEN_P(CG(current_namespace)) + 1 + Z_STRLEN_P(name) + 1);
//
