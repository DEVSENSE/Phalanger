...
ZEND_METHOD(Closure, __invoke) /* {{{ */
{
	...
	efree((void*)func->internal_function.function_name);
	efree(func);
}
/* }}} */
...
static zend_function *zend_closure_get_method(zval **object_ptr, char *method_name, int method_len TSRMLS_DC) /* {{{ */
{
	...
	lc_name = (char *)do_alloca(method_len + 1, use_heap);
...
static zend_object_value zend_closure_new(zend_class_entry *class_type TSRMLS_DC) /* {{{ */
{
	...
	closure = (zend_closure *)emalloc(sizeof(zend_closure));
...
{
	HashTable *target = (HashTable *)va_arg(args, int*);
	zend_bool is_ref;
