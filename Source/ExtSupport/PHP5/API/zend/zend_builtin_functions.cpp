...
//
// zend_builtin_functions.cpp
// - this is slightly modified zend_builtin_functions.c, originally PHP 5.3.3 source files
//
...
ZEND_FUNCTION(func_get_arg)
{
	...
	arg = (zval*)*(p-(arg_count-requested_offset));
	*return_value = *arg;
	...
	if (zend_hash_exists(&ce->function_table, lcname, method_len+1)) {
		...
		&& (func = Z_OBJ_HT_P(klass)->get_method(&klass, method_name, method_len TSRMLS_CC)) != NULL
			...
			&& (func->common.fn_flags & ZEND_ACC_CALL_VIA_HANDLER) != 0
				...
				efree(lcname);
				efree((void*)((zend_internal_function*)func)->function_name);
				efree(func);
				return;
			}
			efree(lcname);
...
ZEND_FUNCTION(class_exists)
{
	...
	if (!autoload) {
		...
		lc_name = (char*)do_alloca(class_name_len + 1, use_heap);
...
ZEND_FUNCTION(interface_exists)
{
	...
	if (!autoload) {
		...
		lc_name = (char*)do_alloca(iface_name_len + 1, use_heap);
...
ZEND_FUNCTION(restore_error_handler)
{
	...
	if (zend_ptr_stack_num_elements(&EG(user_error_handlers))==0) {
		...
		EG(user_error_handler) = (zval*)zend_ptr_stack_pop(&EG(user_error_handlers));
	}
	RETURN_TRUE;
}
/* }}} */
...
ZEND_FUNCTION(restore_exception_handler)
{
	...
	if (zend_ptr_stack_num_elements(&EG(user_exception_handlers))==0) {
		...
		EG(user_exception_handler) = (zval*)zend_ptr_stack_pop(&EG(user_exception_handlers));
	}
	RETURN_TRUE;
}
/* }}} */
...
{
	TMP_UNSUPPORTED_MSG("get_defined_functions");
...
ZEND_FUNCTION(get_defined_constants)
{
	...
	if (categorize) {
		...
		modules = (zval**)ecalloc(zend_hash_num_elements(&module_registry) + 2, sizeof(zval *));
		module_names = (char**)emalloc((zend_hash_num_elements(&module_registry) + 2) * sizeof(char *));
