...
//
// zend_object_handlers.cpp
// - this is slightly modified zend_object_handlers.c, originally PHP 5.3.3 source files
//
...
static union _zend_function *zend_std_get_method(zval **object_ptr, char *method_name, int method_len TSRMLS_DC) /* {{{ */
{
	...
	lc_method_name = (char*)do_alloca(method_len+1, use_heap);
...
{
	zend_internal_function *callstatic_user_call = (zend_internal_function*)emalloc(sizeof(zend_internal_function));
