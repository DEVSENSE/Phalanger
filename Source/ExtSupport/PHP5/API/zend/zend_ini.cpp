...
//
// zend_ini.cpp
// - this is slightly modified zend_ini.c, originally PHP 5.3.3 source files
//
...
//In PHP 5.3.3 there is just a HashTable, but in IniConfig.cpp at ExtSupport.PHP4 there is TsHashTable.
...
static int zend_ini_refresh_cache_one_module(zend_ini_entry *p, int module_number TSRMLS_DC)
{
	if (p->on_modify) 
	{
		...
		{
#ifdef DEBUG
			Debug::WriteLine("PHP5TS", "zend_ini_refresh_cache_one_module: about to call on_modify handler");
#endif
...
{
	//originaly it was zend_ts_hash_apply_with_argument
	zend_hash_apply_with_argument(registered_zend_ini_directives, (apply_func_arg_t)zend_ini_refresh_ini_entry_one_module,
		(void *)(long)module_number TSRMLS_CC);
}
