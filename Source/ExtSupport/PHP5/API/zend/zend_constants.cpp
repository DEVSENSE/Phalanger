...
//
// zend_constants.cpp
// - this is slightly modified zend_constants.c, originally PHP 5.3.3 source files
//
...
ZEND_API int zend_get_constant_ex(const char *name, uint name_len, zval *result, zend_class_entry *scope, ulong flags TSRMLS_DC)
{
	...
	if ((colon = (char*)zend_memrchr(name, ':', name_len)) &&
	    colon > name && (*(colon - 1) == ':')) {
	...
	/* non-class constant */
	if ((colon = (char*)zend_memrchr(name, '\\', name_len)) != NULL) {
		...
		lcname = (char*)erealloc(lcname, prefix_len + 1 + const_name_len + 1);
		lcname[prefix_len] = '\\';
