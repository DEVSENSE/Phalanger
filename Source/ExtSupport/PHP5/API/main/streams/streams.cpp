...
//
// streams.cpp
// - this is slightly modified streams.c, originally PHP 5.3.3 source files
//
...
static void php_stream_fill_read_buffer(php_stream *stream, size_t size TSRMLS_DC)
{
	...
	if (stream->readfilters.head) {
		...
		chunk_buf = (char*)emalloc(stream->chunk_size);
...
PHPAPI char *php_stream_get_record(php_stream *stream, size_t maxlen, size_t *returned_len, char *delim, size_t delim_len TSRMLS_DC)
{
	...
	if (delim_len == 0 || !delim) {
		...
		if (delim_len == 1) {
			e = (char*)memchr(stream->readbuf + stream->readpos, *delim, seek_len);
		} else {
			e = (char*)php_memnstr((char*)stream->readbuf + stream->readpos, delim, delim_len, (char*)(stream->readbuf + stream->readpos + seek_len));
		}
...
PHPAPI size_t _php_stream_passthru(php_stream * stream STREAMS_DC TSRMLS_DC)
{
	...
	if (php_stream_mmap_possible(stream)) {
		...
		//why PHP folks assigning php_stream_mmap_access_t to php_stream_mmap_operation_t ???
...
PHPAPI size_t _php_stream_copy_to_mem(php_stream *src, char **buf, size_t maxlen, int persistent STREAMS_DC TSRMLS_DC)
{
	...
	if (maxlen > 0) {
		ptr = *buf = (char*)pemalloc_rel_orig(maxlen + 1, persistent);
