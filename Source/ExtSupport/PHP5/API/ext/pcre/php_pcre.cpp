...
//
// php_pcre.cpp
// - this is slightly modified php_pcre.c, originally PHP 5.3.3 source files
//
 ...
 */
PCRE_API pcre_cache_entry* pcre_get_compiled_regex_cache(char *regex, int regex_len TSRMLS_DC)
{
	pcre				*re = NULL;
	pcre_extra			*extra;
	int					 coptions = 0;
 ...
 */
PCRE_API pcre* pcre_get_compiled_regex(char *regex, pcre_extra **extra, int *preg_options TSRMLS_DC)
{
 ...
 */
PCRE_API pcre* pcre_get_compiled_regex_ex(char *regex, pcre_extra **extra, int *preg_options, int *compile_options TSRMLS_DC)
{
	pcre_cache_entry * pce = pcre_get_compiled_regex_cache(regex, strlen(regex) TSRMLS_CC);
...
/* {{{ php_pcre_match_impl() */
PCRE_API void php_pcre_match_impl(pcre_cache_entry *pce, char *subject, int subject_len, zval *return_value,
...
/* {{{ php_pcre_replace
 */
PCRE_API char *php_pcre_replace(char *regex,   int regex_len,
...
PCRE_API char *php_pcre_replace_impl(pcre_cache_entry *pce, char *subject, int subject_len, zval *replace_val, 
	int is_callable_replace, int *result_len, int limit, int *replace_count TSRMLS_DC)
{
	...
	result = (char*)safe_emalloc(alloc_len, sizeof(char), 0);
	...
	while (1) {
		...
		if (count > 0 && (limit == -1 || limit > 0)) {
			...
			if (new_len + 1 > alloc_len) {
				...
				new_buf = (char*)emalloc(alloc_len);
			...
			*result_len += match-piece;
			...
			if (g_notempty != 0 && start_offset < subject_len) {
				...
				if (new_len + 1 > alloc_len) {
					...
					new_buf = (char*)safe_emalloc(alloc_len, sizeof(char), 0);
					memcpy(new_buf, result, *result_len);
					efree(result);
					result = new_buf;
				}
...
/* {{{ php_pcre_split
 */
PCRE_API void php_pcre_split_impl(pcre_cache_entry *pce, char *subject, int subject_len, zval *return_value,
...
static PHP_FUNCTION(preg_quote)
{
	...
	out_str = (char*)safe_emalloc(4, in_str_len, 1);
	...
	*q = '\0';
	...
	RETVAL_STRINGL((char*)erealloc(out_str, q - out_str + 1), q - out_str, 0);
}
/* }}} */
...
PCRE_API void  php_pcre_grep_impl(pcre_cache_entry *pce, zval *input, zval *return_value, long flags TSRMLS_DC) /* {{{ */
{
...
#pragma region Phalanger change
PCRE_API zend_module_entry *pcre_get_module()
{
	return &pcre_module_entry;
}
#pragma endregion
