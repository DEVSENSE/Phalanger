...
//
// fileinfo.cpp
// - this is slightly modified fileinfo.c, originally PHP 5.3.3 source files
//
...
#include "../standard/file.h" /* needed for context stuff */
#include "php_fileinfo.h"
#include "../../main/fopen_wrappers.h" /* needed for is_url */
...
{ \
	struct finfo_object *obj = (struct finfo_object*)zend_object_store_get_object(object TSRMLS_CC); \
	finfo = obj->ptr; \
	if (!finfo) { \
...
PHP_FILEINFO_API zend_object_value finfo_objects_new(zend_class_entry *class_type TSRMLS_DC)
{
	...
	intern = (finfo_object*)emalloc(sizeof(struct finfo_object));
...
#pragma region Phalanger change
	zend_module_entry * pcre_module = pcre_get_module();
	ts_allocate_id(pcre_module->globals_id_ptr, pcre_module->globals_size, (ts_allocate_ctor) pcre_module->globals_ctor, (ts_allocate_dtor) pcre_module->globals_dtor);
#pragma endregion
...
#pragma region Phalanger change
static PHP_RINIT_FUNCTION(finfo)
{
	zend_module_entry * pcre_module = pcre_get_module();
	void * pcre_globals = (*((void ***)tsrm_ls))[TSRM_UNSHUFFLE_RSRC_ID(*(pcre_module->globals_id_ptr))];
	//pcre_module->globals_ctor(pcre_globals TSRMLS_CC);
	return SUCCESS;
}

static PHP_RSHUTDOWN_FUNCTION(finfo)
{
	zend_module_entry * pcre_module = pcre_get_module();
	void * pcre_globals = (*((void ***)tsrm_ls))[TSRM_UNSHUFFLE_RSRC_ID(*(pcre_module->globals_id_ptr))];
	//pcre_module->globals_dtor(pcre_globals TSRMLS_CC);
	return SUCCESS;
}
...
PHP_FUNCTION(finfo_open)
{
	...
	finfo = (php_fileinfo*)emalloc(sizeof(struct php_fileinfo));
