...
//
// string.cpp
// - this is slightly modified string.c, originally PHP 5.3.3 source files
//
...
PHP_FUNCTION(wordwrap)
{
	...
	if (breakcharlen == 1 && !docut) {
		...
		if (linelength > 0) {
			...
			newtext = (char*)safe_emalloc(chk, breakcharlen, textlen + 1);
			alloced = textlen + chk * breakcharlen + 1;
		} else {
			chk = textlen;
			alloced = textlen * (breakcharlen + 1) + 1;
			newtext = (char*)safe_emalloc(textlen, (breakcharlen + 1), 1);
		}
		...
		for (current = 0; current < textlen; current++) {
			if (chk <= 0) {
				...
				newtext = (char*)erealloc(newtext, alloced);
		...
		/* free unused memory */
		newtext = (char*)erealloc(newtext, newtextlen+1);
...
#define EXPLODE_ALLOC_STEP 64
	...
	if (p2 == NULL) {
		...
		*/
		...
		long i, to_return;
		char **positions = (char**)emalloc(allocated * sizeof(char *));

		positions[found++] = p1;
		do {
			if (found >= allocated) {
				allocated = found + EXPLODE_ALLOC_STEP;/* make sure we have enough memory */
				positions = (char**)erealloc(positions, allocated*sizeof(char *));
			}
...
PHP_FUNCTION(pathinfo)
{
	...
	if ((opt & PHP_PATHINFO_EXTENSION) == PHP_PATHINFO_EXTENSION) {
		...
		p = (char*)zend_memrchr(ret, '.', ret_len);
...
PHP_FUNCTION(strrchr)
{
	...
	if (Z_TYPE_P(needle) == IS_STRING) {
		found = (char*)zend_memrchr(haystack, *Z_STRVAL_P(needle), haystack_len);
	} else {
		char needle_chr;
		...
		found = (char*)zend_memrchr(haystack,  needle_chr, haystack_len);
	}
...
static char *php_chunk_split(char *src, int srclen, char *end, int endlen, int chunklen, int *destlen)
{
	...
	dest = (char*)safe_emalloc((int)out_len, sizeof(char), 0);
...
PHP_FUNCTION(quotemeta)
{
	...
	str = (char*)safe_emalloc(2, old_len, 1);
	...
	*q = 0;
	...
	RETURN_STRINGL((char*)erealloc(str, q - str + 1), q - str, 0);
}
/* }}} */
...
{
	char flags[256];
	char *new_str = (char*)safe_emalloc(4, (length?length:(length=strlen(str))), 1);
	char *source, *target;
	char *end;
	char c;
	int  newlen;
...
PHPAPI int php_char_to_str_ex(char *str, uint len, char from, char *to, int to_len, zval *result, int case_sensitivity, int *replace_count)
{
	...
	if (case_sensitivity) {
		char *p = str, *e = p + len;
		while ((p = (char*)memchr(p, from, (e - p)))) {
			char_count++;
			p++;
		}
	} else {
	...
	Z_STRVAL_P(result) = target = (char*)safe_emalloc(char_count, to_len, len + 1);
...
PHP_FUNCTION(nl2br)
{
	...
	tmp = target = (char*)emalloc(new_length + 1);
				...
				if (state == 0) {
					...
					if (allow) {
						if (tp - tbuf >= PHP_TAG_BUF_SIZE) {
							pos = tp - tbuf;
							tbuf = (char*)erealloc(tbuf, (tp - tbuf) + PHP_TAG_BUF_SIZE + 1);
							tp = tbuf + pos;
						}
						*(tp++) = '<';
				 	}
...
PHP_FUNCTION(substr_count)
{
	...
	if (needle_len == 1) {
		...
		while ((p = (char*)memchr(p, cmp, endp - p))) {
			count++;
			p++;
		}
	} else {
