...
//
// html.cpp
// - this is slightly modified html.c, originally PHP 5.3.3 source files
//
	...
	if (all) {
		...
		for (j = 0; entity_map[j].charset != cs_terminator; j++) {
			...
			for (k = entity_map[j].basechar; k <= entity_map[j].endchar; k++) {
				...
				entity_length = slprintf((char*)entity, sizeof(entity), "&%s;", entity_map[j].table[k - entity_map[j].basechar]);
				...
				if (php_memnstr(ret, (char*)entity, entity_length, (char*)ret+retlen)) {
					replaced = php_str_to_str(ret, retlen, (char*)entity, entity_length, (char*)replacement, replacement_len, &retlen);
					efree(ret);
					ret = replaced;
				}
			}
...
PHPAPI char *php_escape_html_entities_ex(unsigned char *old, int oldlen, int *newlen, int all, int quote_style, char *hint_charset, zend_bool double_encode TSRMLS_DC)
{
	...
	while (i < oldlen) {
		...
		if (len + 16 > maxlen)
			replaced = (char*)erealloc (replaced, maxlen += 128);
		...
		if (all) {
			...
			if (matches_map) {
				...
				replaced[len++] = '&';
				strlcpy(replaced + len, (char*)rep, maxlen);
				len += l;
				replaced[len++] = ';';
			}
		}
...
encode_amp:
					...
					char *e = (char*)memchr(old + i, ';', oldlen - i);
					char *s = (char*)old + i;
...
static void php_html_entities(INTERNAL_FUNCTION_PARAMETERS, int all)
{
	...
	replaced = php_escape_html_entities_ex((unsigned char*)str, str_len, &len, all, quote_style, hint_charset, double_encode TSRMLS_CC);
