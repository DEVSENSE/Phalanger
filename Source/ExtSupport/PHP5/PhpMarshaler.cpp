...
namespace PHP
{
	namespace ExtManager
	{
		...
		void PhpMarshaler::MarshalManagedArrayToNativeArray(IDictionaryEnumerator ^enumerator, HashTable *ht,
			bool probeStringKeys)
		{
		...
		IntPtr PhpMarshaler::MarshalManagedToNative(Object ^ManagedObj)
		{
			...
			{
				((zval *)ret_val.ToPointer())->refcount__gc++;
			...
			else switch (Type::GetTypeCode(ManagedObj->GetType()))
			{
				...
				{
					System::String ^string = static_cast<System::String ^>(ManagedObj);

					array<unsigned char> ^bytes = Request::AppConf->Globalization->PageEncoding->GetBytes(string);
					int length = bytes->Length;
				...
				case TypeCode::Object:
				{
					...
					// PhpArray
					if ((array = dynamic_cast<PHP::Core::PhpArray ^>(ManagedObj)) != nullptr)
					{
						var->type = IS_ARRAY;
		...
		Object ^PhpMarshaler::MarshalNativeToManaged(IntPtr pNativeData)
		{
			...
			switch (var->type)
			{
				...
				case IS_STRING:
					ret_val = gcnew String(var->value.str.val, 0, var->value.str.len,
