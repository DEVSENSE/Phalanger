...
#include "API/ext/standard/php_fopen_wrappers.h"

//#include "API/zend/zend_constants.h"
//#include "API/zend/zend_API.h"
#include "API/zend/zend_compile.h"
#include "API/zend/zend_modules.h"
#include "API/zend/zend_ini.h"
...
namespace PHP
{
	namespace ExtManager
	{
		...
		Object ^Function::Invoke(bool retValueUsed, PhpObject ^self, array<Object ^> ^args, array<int> ^refInfo)
		{
			// get current request
			Request ^request = Request::GetCurrentRequest();
			...
			try
			{
				...
				try
				{
					...
					for (int i = 0; i < num_args; i++)
					{
						...
						{
							request->CurrentInvocationContext.FunctionPhpArgs[i]->is_ref__gc = 1;
						}
					}
...
//				}
				...
				// check whether all arguments that are forced to be passed by reference actually are
				const _zend_arg_info *info = functionEntry->arg_info;
				if (info != NULL)
				{
		...
		Method::Method(Class ^cls, const zend_function_entry *func)
		...
		Object ^Method::Invoke(bool retValueUsed, PhpObject ^self, array<Object ^> ^args, array<int> ^refInfo)
		{
			...
			Object ^ret_val = Function::Invoke(retValueUsed, self, args, refInfo);
...
#ifdef PHP4TS
		// Invokes the handler.
		Object ^CallerMethod::InvokeHandler(bool retValueUsed, PHP::Core::PhpObject ^self, array<Object ^> ^args,
			array<int> ^refInfo, String ^methodName)
		{
			...
			try
			{
				return Method::Invoke(retValueUsed, self, args, refInfo, &property_reference);
			}
			finally
			{
		...
		Object ^CallerMethod::InvokeHandler(bool retValueUsed, PHP::Core::PhpObject ^self, array<Object ^> ^args,
			array<int> ^refInfo, String ^methodName)
		{
			return Method::Invoke(retValueUsed, self, args, refInfo);
		}

		//CallerMethod implementation

		// Invokes the handler as a <c>__call</c> method - name of the method to invoke is expected in argument 0.
		Object ^CallerMethod::Invoke(bool retValueUsed, PHP::Core::PhpObject ^self, array<Object ^> ^args,
			array<int> ^refInfo)
		{
			...
			Object ^ret_val = InvokeHandler(retValueUsed, self, new_args, ref_info, 
		...
		Object ^ConstructorMethod::Invoke(bool retValueUsed, PHP::Core::PhpObject ^self, array<Object ^> ^args,
			array<int> ^refInfo, zend_property_reference *propertyReference)
		{
			return callerMethod->InvokeHandler(retValueUsed, self, args, refInfo, GetFunctionName());
		}
		...
		Object ^GetterMethod::Invoke(bool retValueUsed, PHP::Core::PhpObject ^self, array<Object ^> ^args,
			array<int> ^refInfo, zend_property_reference *propertyReference)
		{
		...
		Object ^SetterMethod::Invoke(bool retValueUsed, PHP::Core::PhpObject ^self, array<Object ^> ^args,
			array<int> ^refInfo, zend_property_reference *propertyReference)
		{
		...
		Class::Class(Module ^mod, zend_class_entry *entry, zend_class_entry *parentEntry)
		{
			...
			*classEntry->refcount = 1;
			...
			const zend_function_entry *func = classEntry->builtin_functions;
...
#pragma warning (pop)
				...
				_zend_function* func = classEntry->__call;
				...
				func = classEntry->__get;
				if ((func != NULL) && (func->type == ZEND_INTERNAL_FUNCTION))
				{
					// create an artificial __get method
					SpecialMethod ^method = gcnew SpecialMethod(PHP::Core::PhpObject::SpecialMethodNames::Get.Value, this, func->internal_function.handler);
					methods->Add(method->GetFunctionName(), method);
				}

				func = classEntry->__set;
				if ((func != NULL) && (func->type == ZEND_INTERNAL_FUNCTION))
				{
					// create an artificial __set method
					SpecialMethod ^method = gcnew SpecialMethod(PHP::Core::PhpObject::SpecialMethodNames::Set.Value, this, func->internal_function.handler);
					methods->Add(method->GetFunctionName(), method);
				}

				func = classEntry->constructor;
				if (!methods->ContainsKey(PHP::Core::PhpObject::SpecialMethodNames::Construct.Value) &&
						!methods->ContainsKey(GetClassName()->ToLower()) &&
						(func != NULL) && (func->type == ZEND_INTERNAL_FUNCTION))
				{
					// create an artificial construct method
					SpecialMethod ^method = gcnew SpecialMethod(PHP::Core::PhpObject::SpecialMethodNames::Construct.Value, this, func->internal_function.handler);
					methods->Add(method->GetFunctionName(), method);
				}

				func = classEntry->destructor;
				if (!methods->ContainsKey(PHP::Core::PhpObject::SpecialMethodNames::Destruct.Value) &&
						(func != NULL) && (func->type == ZEND_INTERNAL_FUNCTION))
				{
					// create an artificial destruct method
					SpecialMethod ^method = gcnew SpecialMethod(PHP::Core::PhpObject::SpecialMethodNames::Destruct.Value, this, func->internal_function.handler);
					methods->Add(method->GetFunctionName(), method);
				}

				func = classEntry->clone;
				if ((func != NULL) && (func->type == ZEND_INTERNAL_FUNCTION))
				{
					// create an artificial destruct method
					SpecialMethod ^method = gcnew SpecialMethod(PHP::Core::PhpObject::SpecialMethodNames::Clone.Value, this, func->internal_function.handler);
					methods->Add(method->GetFunctionName(), method);
				}

				func = classEntry->__unset;
				if ((func != NULL) && (func->type == ZEND_INTERNAL_FUNCTION))
				{
					// create an artificial destruct method
					SpecialMethod ^method = gcnew SpecialMethod(PHP::Core::PhpObject::SpecialMethodNames::Unset.Value, this, func->internal_function.handler);
					methods->Add(method->GetFunctionName(), method);
				}

				func = classEntry->__isset;
				if ((func != NULL) && (func->type == ZEND_INTERNAL_FUNCTION))
				{
					// create an artificial destruct method
					SpecialMethod ^method = gcnew SpecialMethod(PHP::Core::PhpObject::SpecialMethodNames::Isset.Value, this, func->internal_function.handler);
					methods->Add(method->GetFunctionName(), method);
				}

				func = classEntry->__tostring;
				if ((func != NULL) && (func->type == ZEND_INTERNAL_FUNCTION))
				{
					// create an artificial destruct method
					SpecialMethod ^method = gcnew SpecialMethod(PHP::Core::PhpObject::SpecialMethodNames::Tostring.Value, this, func->internal_function.handler);
					methods->Add(method->GetFunctionName(), method);
				}
		...
		IniEntry ^Module::GetConfigEntry(String ^key)
		{
			...
			ExtensionLibraryDescriptor ^descriptor =
				dynamic_cast<ExtensionLibraryDescriptor ^>(request->ExtConfig[GetFileName()]);
		...
		DynamicModule::DynamicModule(String ^_path, String ^_fileName, bool _earlyInit)
		{
			...
			try
			{
				...
				const zend_function_entry *func = moduleEntry->functions;
