...
#pragma warning (pop)
			...
			/// </summary>
			PHP::Core::OrderedHashtable<String ^> ^TransientClasses;
			...
			/// </remarks>
			OrderedHashtable<String ^> ^ExtConfig;
		...
		public:
			...
			static Request ^GetCurrentRequest(bool wantException, bool createNew)
			{
				...
				Request ^request = static_cast<Request ^>(CallContext::GetData(RequestThreadSlotName));
				if (request == nullptr && createNew) return gcnew Request();
				return request;
			}
		...
		public:
			...
			static String ^RequestThreadSlotName = "RemoteExtManRequest5";
